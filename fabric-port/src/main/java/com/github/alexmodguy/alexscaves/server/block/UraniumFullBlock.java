...
public class UraniumFullBlock extends RotatedPillarBlock {
    public static final MapCodec<UraniumFullBlock> CODEC = simpleCodec(UraniumFullBlock::new);

    public UraniumFullBlock() {
        this(Properties.of().mapColor(MapColor.COLOR_LIGHT_GREEN).strength(3.5F).lightLevel(state -> 4).emissiveRendering((state, level, pos) -> true).sound(ACSoundTypes.URANIUM));
    }

    private UraniumFullBlock(Properties properties) {
        super(properties);
    }
